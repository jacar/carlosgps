...
(function() {
  ...
  document.getElementById('camSnapshotBtn')?.addEventListener('click', () => showToast('📸 Capturas guardadas en el servidor', 'success'));

  const VIDEO_SERVER_IP = '66.97.42.27';
  const VIDEO_SERVER_PORT = '8081';
  ...
  document.getElementById('camAutoLinkBtn')?.addEventListener('click', () => {
    ...
    [0, 1, 2, 3].forEach(i => {
      // Logic for CMSV6 standard HLS: http://IP:PORT/hls/IMEI/CHANNEL.m3u8
      const url = `http://${VIDEO_SERVER_IP}:${VIDEO_SERVER_PORT}/hls/${device.imei}/${i+1}.m3u8`;
      nextCfg[i] = { url };
